...
public class MainWindow : Gtk.ApplicationWindow {
  ...
  public MainWindow (Gtk.Application app, Account? account = null) {
    ...
    this.set_icon_name ("nextbird");
    this.delete_event.connect (window_delete_cb);

    this.headerbar = new Gtk.HeaderBar ();
    headerbar.set_title ("Nextbird");
    ...
    compose_tweet_button.add (new Gtk.Image.from_icon_name ("nextbird-compose-symbolic",
                                                            Gtk.IconSize.BUTTON));
    ...
    ((Nextbird)app).account_added.connect ((new_acc) => {
    ...
    ((Nextbird)app).account_removed.connect ((acc) => {
      var entries = account_list.get_children ();
      foreach (Gtk.Widget ule in entries)
        if (ule is UserListEntry &&
  ...
  public void change_account (Account? account) {
    ...
    Nextbird cb = (Nextbird) GLib.Application.get_default ();
    ...
    if (account != null && account.screen_name != Account.DUMMY) {
      ...
      this.set_title ("Nextbird - @%s".printf (account.screen_name));
      ...
      this.title_label.label = "Nextbird";
      this.set_title ("Nextbird");
  ...
  private void account_row_activated_cb (Gtk.ListBoxRow row) {
    ...
    Nextbird cb = (Nextbird)this.get_application ();
  ...
  private void account_info_changed (string        screen_name,
    ...
    this.set_title ("Nextbird - @%s".printf (screen_name));
  }
