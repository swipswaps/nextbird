...
public class Account : GLib.Object {
  ...
  private async bool update_avatar (string url = "") {
    ...
    if (url.length > 0) {
      ...
      SOUP_SESSION.queue_message (msg, (_s, _msg) => {
        ...
        this.avatar_url = url;
        Nextbird.db.update ("accounts").val ("avatar_url", url).where_eqi ("id", id).run ();
  ...
  public void save_info () {
    db.replace ("info").vali64 ("id", id)
                       ...
                       .val ("name", name)
                       .run ();
    Nextbird.db.replace ("accounts").vali64 ("id", id)
                                    .val ("screen_name", screen_name)
                                    .val ("name", name)
  ...
  /**
   * Look up the accounts. Each account has a <id>.db in ~/.nextbird/accounts/
  ...
  private static void lookup_accounts () {
    ...
    Nextbird.db.select ("accounts").cols ("id", "screen_name", "name", "avatar_url").run ((vals) => {
