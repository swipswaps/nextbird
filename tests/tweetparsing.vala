  ...
  "retweeted_status" : {
    ...
    "quoted_status" : {
      ...
      "user" : {
        ...
        "screen_name" : "baedert",
        "location" : "",
        "description" : "Nextbird developer by night, silently judging people on the train by day.",
        ...
        "entities" : {
          "url" : {
            "urls" : [
              {
                "url" : "https://t.co/yGvX7Nf6i3",
                "expanded_url" : "http://nextbird.baedert.org",
                "display_url" : "nextbird.baedert.org",
                "indices" : [
                  0,
                  23
                ]
...
const string TD8 = """
{
  ...
  "full_text" : "RT @nextbirdgtk: @baedert and @nextbirdclient so?",
  "truncated" : false,
  "display_text_range" : [
    0,
    49
  ...
  "retweeted_status" : {
    ...
    "full_text" : "@baedert and @nextbirdclient so?",
    "truncated" : false,
    "display_text_range" : [
      9,
      32
    ...
    "in_reply_to_screen_name" : "nextbirdgtk",
    "user" : {
      "id" : 993713617,
      "id_str" : "993713617",
      "name" : "Z??!@*(&*²³¤²³¤",
...
void media_count () {
  ...
  t.load_from_json (root, 0, now);
  assert (t.get_screen_name () == "nextbirdgtk");
...
void double_media () {
  ...
  // See https://github.com/baedert/nextbird/issues/627
...
void rt_reply () {
   ...
   * to 2 users (namely @nextbirdgtk and @baedert).
   * @baedert is mentioned directly in the tweet text, before the display_range_start
   * and @nextbirdgtk as the in_reply_to_screen_name embedded in the retweeted tweet.
   *
   * The result we want is that the CbTweet is a reply to all the
   * users the retweeted tweet was a reply to, so both @nextbirdgtk and @baedert.
   */
  assert (t.retweeted_tweet.reply_users.length == 2);
  /* It's a direct reply to @nextbirdgtk, so that should be first */
  assert (t.retweeted_tweet.reply_users[0].screen_name == "nextbirdgtk");
  ...
  assert (reply_users[0].screen_name == "nextbirdgtk");
